...
- The current signed candidate is on `codex/ios-appstore-build5-20260805` in a clean isolated worktree. It combines the build-4 logo/image/startup work with the FI/IE/AU region-sheet fixes and disables iOS search autocorrection so queries such as `sabre` are preserved. It has not been merged into `main` or deployed.
- `npm run verify` passed end to end on 2026-08-05: 40/40 tests, config, release assets, typecheck, Expo Doctor 20/20, live exchange rates, 5,719 products, 84,072 price-history rows, a 200-item image-complete startup preview, AU 16, and an iOS export of 1,497 modules / 5.4 MB. The command ended with `verify_local_ok`.
- Expo account: `noir-madlax`; EAS project: `@noir-madlax/geardrop` (`ead43b0e-5dbf-44a2-838e-f65db29abb30`).
- EAS production build `e88a4ac2-88f8-4332-9b77-7ee9127cd355` finished successfully as App `1.0.0`, build `5`, from source commit `392a7e957fd00f06e6210f5f245272c35aeededa`. Its 30,079,040-byte signed IPA has SHA-256 `196a7b09a0e7c830d53c04588be418038b12583f2ac36b5fc67a6b9bbc070a0c`; codesign and Info.plist were independently validated. Apple returned `VERIFY SUCCEEDED with no errors`, `UPLOAD SUCCEEDED with no errors`, then `processingState=VALID` / `APP_STORE_ELIGIBLE`.
...
- Bundle ID `dev.100app.geardrop`, version `1.0.0`, submitted build number `5`, portrait orientation.
...
- TestFlight build `1.0.0 (5)` is `IN_BETA_TESTING` and `READY_FOR_BETA_SUBMISSION`. Internal group `GearDrop Internal` now contains build 2, build 3, build 4, and build 5 with 3 testers. The group relationship, tester count, and iOS version relationship were independently read back through Apple's API after the build-5 writes.
- Post-build-5 branch `codex/ios-pro-offer-code-20260805` adds an Apple system Offer Code entry to the paywall. It keeps RevenueCat `CustomerInfo` as the only Pro authority and contains no local invite-code unlock. App Store Connect now has active free lifetime Offer `PRO_INVITE_LIFETIME_20260805` plus a 100-code SANDBOX batch expiring 2027-02-01; 99 codes are in the restricted local delivery artifact and one is isolated as reserve. The UI change is not present in build 5 or any signed/TestFlight artifact yet, the sandbox redemption path is not device-verified, and no production code exists.
...
1. **Build-5 device smoke:** install or update to build 5 from TestFlight and verify FI/IE/AU region names/flags/scrolling/persistence, the unchanged `sabre` search query, first logo, homepage image rendering, and 200-item startup preview before the complete catalog.
2. **IAP transaction proof:** on build 5 verify localized prices, monthly/annual/lifetime purchase, cancellation, pending purchase, entitlement activation, reinstall/restore, no-purchase restore, and offline recovery using the matrix in `IAP_SETUP.md`.
...
5. **Invite-code acceptance:** the permanent lifetime Offer and sandbox codes now exist. Generate a signed next build, then verify invalid/valid redemption, automatic RevenueCat entitlement activation, relaunch, and restore. Production one-time-use codes remain blocked until the app is Ready for Distribution and the lifetime IAP is Approved; Apple requires at least 500 per production batch.
...
Build 5, Apple upload, processing, TestFlight internal-group assignment, and App Store version attachment are complete. Do not submit for App Review until the build-5 signed transaction matrix and final review screenshots are complete. The exact product and sandbox steps are in `IAP_SETUP.md`.
