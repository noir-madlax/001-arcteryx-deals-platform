...
- Post-build-5 branch `codex/ios-pro-offer-code-20260805` adds an Apple system Offer Code entry to the paywall. It keeps RevenueCat `CustomerInfo` as the only Pro authority and contains no local invite-code unlock. This change is locally verified but is not present in build 5 or any signed/TestFlight artifact yet; no production Offer Code has been created.
...
5. **Invite-code acceptance:** choose permanent versus time-limited access and one-time versus shared custom codes, create a sandbox Apple Offer Code, then verify invalid/valid redemption, automatic RevenueCat entitlement activation, relaunch, and restore on the signed next build.
