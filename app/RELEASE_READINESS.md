...
- TestFlight build `1.0.0 (2)` is visible as `Ready to Submit` and expires in 90 days. Internal group `GearDrop Internal` contains build 2 and has automatic distribution disabled. The build test notes read back as saved; the existing Account Holder was invited and the group independently read back as `1 Tester` / `1 Build`. The requested Jason email is not yet an eligible internal tester because it is not present in App Store Connect Users and Access.
...
1. **Internal beta access:** the group, build, test notes, and first invitation are complete. Adding Jason requires an exact first/last name and an approved App Store Connect role before creating the team user; after that user accepts, add it to `GearDrop Internal`.
